...
graph TB
    ...
    subgraph LeapMentor Backend
        ...
        MW[Middleware<br/>auth · rate-limit · upload · sanitize]
        RT[Routes<br/>/api/v1/*]
        CT[Controllers]
    ...
    subgraph External Services
        MDB[(MongoDB Atlas)]
        RDS[(Redis<br/>Rate limiting + cache)]
...
sequenceDiagram
    ...
    C->>MW: HTTP Request
    MW->>MW: JWT Auth + Rate Limit + Sanitize
    MW->>CT: req, res, next
...
flowchart TD
    ...
    B --> C{Mentor reviews}
    C -->|Accept| D[PATCH /connect-requests/:id]
    C -->|Reject| E[Request rejected]
    D --> F[POST /escrow/pay — funds held in escrow]
    F --> G[Session takes place]
    G --> H[PATCH /sessions/:id/slots/:slotIndex/status]
...
flowchart TD
    ...
    B --> C[Admin views /admin/reports]
    C --> D{Admin decision}
    D -->|Warn / resolve| E[PATCH /admin/reports/:id]
    D -->|Refund| F[POST /admin/reports/:id/refund]
    D -->|Delete session| G[DELETE /admin/reports/:id/session]
    E --> H[Reporter notified]
    F --> H
    G --> H
```
...
sequenceDiagram
    ...
    U->>BE: POST /auth/password-reset { email }
    BE->>DB: Create VerificationToken (hashed OTP)
    BE->>U: Email with OTP
    U->>BE: POST /auth/password-reset/verification { email, otp }
    BE->>DB: Verify OTP, extend expiry
    U->>BE: POST /auth/password-reset/confirmation { email, otp, newPassword }
