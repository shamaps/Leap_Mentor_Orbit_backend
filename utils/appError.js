...
const handleError = (res, err, context = "unknown") => {
  if (err instanceof AppError) {
    ...
    return res.status(err.status).json({ success: false, message: err.message }); // ← add success:false
  }
  logger.error(`[${context}] unexpected error`, { error: err.message, stack: err.stack });
  return res.status(500).json({ success: false, message: "Internal server error" }); // ← never leak err.message
};
